...
2. **전략적 다중 청킹 기법 (Chunking Strategies) 제공**
   * 문서의 구조적 특성에 맞게 최적의 텍스트 분할을 수행할 수 있도록 **Recursive(재귀적)**, **Semantic(의미론적)**, **Sentence(문장 단위)** 등 3가지 청킹 알고리즘을 지원합니다.

3. **Qdrant Cloud 기반 하이브리드 벡터 DB 구축**
   * **Dense(밀집)** 및 **Sparse(희소)** 벡터 임베딩을 결합한 하이브리드 검색 인프라를 Qdrant Cloud에 구현하였으며, **Payload 인덱싱**을 적용하여 대용량 데이터에서도 고속 필터링이 가능합니다.

4. **고도화된 다중 리트리버 알고리즘 (Retrieval)**
   * 질문의 의도에 맞춰 최적의 컨텍스트를 추출할 수 있도록 단순 **Vector(벡터)**, **Keyword(키워드)**, **Hybrid(하이브리드)** 검색뿐만 아니라 중복을 제거하는 **MMR**, 가상 답변을 활용하는 **HyDE** 기법을 모두 지원합니다.
...
7. **다중 LLM 및 3가지 프롬프트 엔지니어링 전략 지원**
   ...
   * **프롬프트 기법:** 환각을 제어하는 **'기본 4규칙 프롬프트'**, 논리적 추론을 유도하는 **'Zero-shot CoT'**, 예시를 활용하는 **'Few-shot CoT'** 등 총 3가지 모드를 제공합니다.
